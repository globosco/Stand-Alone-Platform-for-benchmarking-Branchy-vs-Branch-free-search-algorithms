    ...
    */
    ...
    if(!strcmp(method, "bbs")){
        if(dataset == "L1_osm_cellids_200M" || dataset == "L2_osm_cellids_200M" || dataset == "L3_osm_cellids_200M"){
            ...
            rs::Builder<uint64_t>rsb(min,max, 15, 95);
            for (const auto& key : A) rsb.AddKey(key);
            rs = rsb.Finalize();
        }else if(dataset == "L2_books_200M"){
            rs::Builder<uint64_t>rsb(min,max, 19, 95);
            for (const auto& key : A) rsb.AddKey(key);
            rs = rsb.Finalize();
        }else if(dataset == "L3_books_200M"  || dataset == "L4_books_200M"){
            rs::Builder<uint64_t>rsb(min,max, 23, 15);
            for (const auto& key : A) rsb.AddKey(key);
            rs = rsb.Finalize();
        }else if(dataset == "L1_fb_200M" || dataset == "L2_fb_200M"  || dataset == "L3_fb_200M"){
            rs::Builder<uint64_t>rsb(min,max, 15,260);
            for (const auto& key : A) rsb.AddKey(key);
            rs = rsb.Finalize();
        }else if(dataset == "L4_fb_200M"){
            rs::Builder<uint64_t>rsb(min,max, 23,35);
            for (const auto& key : A) rsb.AddKey(key);
            rs = rsb.Finalize();
        }else if(dataset == "L1_wiki_ts_200M"  || dataset == "L2_wiki_ts_200M"  ||dataset == "L3_wiki_ts_200M"  || dataset == "L4_wiki_ts_200M"){
            rs::Builder<uint64_t>rsb(min,max, 26, 15);
            for (const auto& key : A) rsb.AddKey(key);
            rs = rsb.Finalize();
        }else{
