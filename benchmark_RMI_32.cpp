  ...
  */
  ...
  if(!strcmp(method, "lower_bound")){
    ...
    ns = util::timing([&]{
        ...
        BranchingBinarySearch<uint32_t>::search(data, Q[pos], 0, A.size()-1);
        }
    });
    ...
    ns = util::timing([&]{
        for (uint32_t pos = 0; pos < Q.size(); pos++) {
        StandardBinarySearch<uint32_t>::search(data, Q[pos], 0, A.size()-1);
        }
    });
    // const auto end = std::chrono::high_resolution_clock::now();
    // timer = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    ...
    ns = util::timing([&]{
        for (uint32_t pos = 0; pos < Q.size(); pos++) {
          // std::cout << pos << std::endl;
            BranchlessBinarySearch<uint32_t>::search(data, Q[pos], 0, A.size()-1);
        }
    });
    // const auto end = std::chrono::high_resolution_clock::now();
    // timer = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    ...
    ns = util::timing([&]{
        for (uint64_t pos = 0; pos < Q.size(); pos++) {
        KBFS_Search<uint32_t>::search(data, Q[pos], 0, A.size()-1, k);
        }
    });
    // const auto end = std::chrono::high_resolution_clock::now();
    // timer = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    ...
    std::vector<Row<uint32_t>> layout;
    layout.resize(A.size());

    auto start = std::chrono::high_resolution_clock::now();
    EytzingerLayoutSearch<uint32_t>::createLayout(data, layout, 0, A.size());
    ...
    ns = util::timing([&]{
        for (uint64_t pos = 0; pos < Q.size(); pos++) {
        EytzingerLayoutSearch<uint32_t>::search(data, Q[pos], 0, A.size()-1);
        }
    });
    ...
    auto start = std::chrono::high_resolution_clock::now();
    EytzingerLayoutSearchNoPrefetch<uint32_t>::createLayout(data, layout, 0, A.size());
    auto end = std::chrono::high_resolution_clock::now();
    buildTimer = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    ...
    ns = util::timing([&]{
        for (uint64_t pos = 0; pos < Q.size(); pos++) {
        EytzingerLayoutSearchNoPrefetch<uint32_t>::search(data, Q[pos], 0, A.size()-1);
        }
    });
    // end = std::chrono::high_resolution_clock::now();
    // timer = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
