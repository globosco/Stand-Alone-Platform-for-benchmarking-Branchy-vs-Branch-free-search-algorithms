...
#include "L1_books_200M_uint32_0_data.h"
#include <math.h>
#include <cmath>
#include <fstream>
#include <filesystem>
#include <iostream>
namespace L1_books_200M_uint32_0 {
bool load(char const* dataPath) {
  {
    std::ifstream infile(std::filesystem::path(dataPath) / "L1_books_200M_uint32_0_L1_PARAMETERS", std::ios::in | std::ios::binary);
...
uint64_t lookup(uint64_t key, size_t* err) {
  ...
  *err = *((uint64_t*) (L1_PARAMETERS + (modelIndex * 24) + 16));
  ...
  return FCLAMP(fpred, 7500.0 - 1.0);
}
} // namespace
