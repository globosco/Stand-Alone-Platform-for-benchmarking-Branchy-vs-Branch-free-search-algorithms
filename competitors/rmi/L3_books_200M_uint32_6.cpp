...
#include "L3_books_200M_uint32_6_data.h"
#include <math.h>
#include <cmath>
#include <fstream>
#include <filesystem>
#include <iostream>
namespace L3_books_200M_uint32_6 {
bool load(char const* dataPath) {
  {
    std::ifstream infile(std::filesystem::path(dataPath) / "L3_books_200M_uint32_6_L1_PARAMETERS", std::ios::in | std::ios::binary);
...
uint64_t lookup(uint64_t key, size_t* err) {
  ...
  *err = *((uint64_t*) (L1_PARAMETERS + (modelIndex * 24) + 16));
  ...
  return FCLAMP(fpred, 1500000.0 - 1.0);
}
} // namespace
